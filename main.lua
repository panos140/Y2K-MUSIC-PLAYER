...
local function getMusicFolder()
    -- getSource() returns the project folder when running from source,
    -- but returns the .exe file itself (not a folder) once fused.
    -- getSourceBaseDirectory() is the mirror image: wrong when running
    -- from source, but correctly returns the folder containing the exe
    -- when fused. So we pick whichever one is actually correct right now.
    local basePath
    if love.filesystem.isFused() then
        basePath = love.filesystem.getSourceBaseDirectory()
    else
        basePath = love.filesystem.getSource()
    end
    return basePath .. "\\music"
end

-- love.filesystem (getDirectoryItems, newFileData, getInfo, etc.) can only
-- "see" files bundled into the exe at build time, or the save directory --
-- NOT arbitrary files written next to the exe on disk, even though our
-- io.open() write above lands there successfully. When fused, we have to
-- explicitly mount that external folder before love.filesystem can read
-- from it. musicVirtualPath is the path to use in every love.filesystem.*
-- call from here on -- "music" when running from source (already visible
-- by default), or "external/music" once fused and mounted.
local musicVirtualPath = "music"

local function setupMusicMount()
    if love.filesystem.isFused() then
        local baseDir = love.filesystem.getSourceBaseDirectory()
        local ok = love.filesystem.mount(baseDir, "external")
        if ok then
            musicVirtualPath = "external/music"
        end
    end
end
        ...
        if isAudioFile(filename) then
            ...
            table.insert(
                playlist,
                {
                    ...
                    path = musicVirtualPath .. "/" .. filename
                }
            )
        end
    end
...
function love.filedropped(file)
    ...
    local info = love.filesystem.getInfo(musicVirtualPath)
    ...
    local output, openErr = io.open(outputPath, "wb")

    if not output then
        dropMessage = "SAVE FAILED: " .. tostring(openErr or "unknown")
        dropMessageTimer = 5
        return
    end
...
function love.draw()
    ...
    if dropMessage ~= "" then
        setColor(COL.btnMagenta)
        love.graphics.rectangle("fill", 60, 205, 340, 60, 10, 10)

        setColor(COL.textLight)
        love.graphics.setFont(smallFont)

        pcall(love.graphics.printf, dropMessage, 70, 218, 320, "center")
    end
